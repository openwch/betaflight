...
static void enableRxIrq(const uartHardware_t *hardware) {
  ...
  HAL_NVIC_SetPriority(hardware->irqn, NVIC_PRIORITY_BASE(hardware->rxPriority),
  ...
  NVIC_InitStructure.NVIC_IRQChannelPreemptionPriority =
      NVIC_PRIORITY_BASE(hardware->rxPriority);
...
uartPort_t *serialUART(uartDevice_t *uartdev, uint32_t baudRate,
...
>>>>>>> 1b9653a92 (dshot 8K,uart4 MSP+DisplayPort function is OK)
    ...
    const ioConfig_t ioCfg =
        IO_CONFIG(GPIO_Mode_AF,
                  ...
                  ((const unsigned[]){GPIO_PuPd_NOPULL, GPIO_PuPd_DOWN,
                                      GPIO_PuPd_UP})[pull]);
#endif
...
void uartConfigureDma(uartDevice_t *uartdev) {
  ...
  const uartDeviceIdx_e uartDeviceIdx =
  ...
  const resourceOwner_e ownerTxRx =
      serialOwnerTxRx(uartPortIdentifier); // rx is always +1
  ...
  if (cfg->txDmaopt != DMA_OPT_UNUSED) {
    dmaChannelSpec = dmaGetChannelSpecByPeripheral(
        DMA_PERIPH_UART_TX, uartDeviceIdx, cfg->txDmaopt);
    if (dmaChannelSpec) {
  ...
  if (cfg->rxDmaopt != DMA_OPT_UNUSED) {
    dmaChannelSpec = dmaGetChannelSpecByPeripheral(
        DMA_PERIPH_UART_RX, uartDeviceIdx, cfg->txDmaopt);
    if (dmaChannelSpec) {
  ...
  if (uartPort->txDMAResource) {
    const dmaIdentifier_e identifier =
        dmaGetIdentifier(uartPort->txDMAResource);
    if (dmaAllocate(identifier, ownerTxRx, ownerIndex)) {
      dmaEnable(identifier);
#if DMA_TRAIT_MUX
      dmaMuxEnable(identifier, uartPort->txDMAMuxId);
#endif
      dmaSetHandler(identifier, uartDmaIrqHandler, hardware->txPriority,
                    (uint32_t)uartdev);
      uartPort->txDMAPeripheralBaseAddr =
          (uint32_t)&UART_REG_TXD(hardware->reg);
    }
  }

  if (uartPort->rxDMAResource) {
    const dmaIdentifier_e identifier =
        dmaGetIdentifier(uartPort->rxDMAResource);
    if (dmaAllocate(identifier, ownerTxRx + 1, ownerIndex)) {
      dmaEnable(identifier);
#if DMA_TRAIT_MUX
      dmaMuxEnable(identifier, uartPort->rxDMAMuxId);
#endif
      uartPort->rxDMAPeripheralBaseAddr =
          (uint32_t)&UART_REG_RXD(hardware->reg);
    }
  }
}
#endif
