...
USB_NOCACHE_RAM_SECTION struct usbd_core_priv {
  ...
  USB_MEM_ALIGNX uint8_t req_data[USB_ALIGN_UP(CONFIG_USBDEV_REQUEST_BUFFER_LEN,
...
static void usbd_class_event_notify_handler(uint8_t busid, uint8_t event,
                                            void *arg);
 ...
 */
static bool usbd_set_endpoint(uint8_t busid,
  ...
  if (ep->bEndpointAddress & 0x80) {
    g_usbd_core[busid].tx_msg[ep->bEndpointAddress & 0x7f].ep_mps =
        USB_GET_MAXPACKETSIZE(ep->wMaxPacketSize);
    g_usbd_core[busid].tx_msg[ep->bEndpointAddress & 0x7f].ep_mult =
        USB_GET_MULT(ep->wMaxPacketSize);
  } else {
    g_usbd_core[busid].rx_msg[ep->bEndpointAddress & 0x7f].ep_mps =
        USB_GET_MAXPACKETSIZE(ep->wMaxPacketSize);
    g_usbd_core[busid].rx_msg[ep->bEndpointAddress & 0x7f].ep_mult =
        USB_GET_MULT(ep->wMaxPacketSize);
  }
 ...
 */
static bool usbd_reset_endpoint(uint8_t busid,
                                const struct usb_endpoint_descriptor *ep) {
 ...
 */
#ifdef CONFIG_USBDEV_ADVANCE_DESC
static bool usbd_get_descriptor(uint8_t busid, uint16_t type_index,
  ...
  case USB_DESCRIPTOR_TYPE_DEVICE:
    ...
    desc = g_usbd_core[busid].descriptors->device_descriptor_callback(
        g_usbd_core[busid].speed);
    if (desc == NULL) {
      found = false;
      break;
  ...
  case USB_DESCRIPTOR_TYPE_CONFIGURATION:
    ...
    desc = g_usbd_core[busid].descriptors->config_descriptor_callback(
        g_usbd_core[busid].speed);
    if (desc == NULL) {
      found = false;
      break;
    }
    desc_len = ((desc[CONF_DESC_wTotalLength]) |
                (desc[CONF_DESC_wTotalLength + 1] << 8));

    g_usbd_core[busid].self_powered =
        (desc[7] & USB_CONFIG_POWERED_MASK) ? true : false;
    g_usbd_core[busid].remote_wakeup_support =
        (desc[7] & USB_CONFIG_REMOTE_WAKEUP) ? true : false;
    break;
  case USB_DESCRIPTOR_TYPE_STRING:
    if (index == USB_OSDESC_STRING_DESC_INDEX) {
      ...
      desc =
          (uint8_t *)g_usbd_core[busid].descriptors->msosv1_descriptor->string;
      ...
      string = g_usbd_core[busid].descriptors->string_descriptor_callback(
          g_usbd_core[busid].speed, index);
      if (string == NULL) {
        found = false;
        break;
      }
  ...
  case USB_DESCRIPTOR_TYPE_DEVICE_QUALIFIER:
    ...
    return false;
#else
    if (g_usbd_core[busid].descriptors->device_quality_descriptor_callback ==
        NULL) {
      found = false;
      break;
    }
    desc = g_usbd_core[busid].descriptors->device_quality_descriptor_callback(
        g_usbd_core[busid].speed);
    if (desc == NULL) {
      found = false;
      break;
  ...
  case USB_DESCRIPTOR_TYPE_OTHER_SPEED:
    if (g_usbd_core[busid].descriptors->other_speed_descriptor_callback ==
        NULL) {
      found = false;
      break;
    }
    desc = g_usbd_core[busid].descriptors->other_speed_descriptor_callback(
        g_usbd_core[busid].speed);
    if (desc == NULL) {
      found = false;
      break;
...
static bool usbd_get_descriptor(uint8_t busid, uint16_t type_index,
  ...
  if ((type == USB_DESCRIPTOR_TYPE_STRING) &&
      (index == USB_OSDESC_STRING_DESC_INDEX)) {
    if (!g_usbd_core[busid].msosv1_desc) {
      return false;
    }

    *data = (uint8_t *)g_usbd_core[busid].msosv1_desc->string;
    // memcpy(*data, (uint8_t *)g_usbd_core[busid].msosv1_desc->string,
    // g_usbd_core[busid].msosv1_desc->string[0]);
    ...
    *data = (uint8_t *)g_usbd_core[busid].bos_desc->string;
    // memcpy(*data, (uint8_t *)g_usbd_core[busid].bos_desc->string,
    // g_usbd_core[busid].bos_desc->string_len);
   ...
   */
  else if ((type == USB_DESCRIPTOR_TYPE_INTERFACE) ||
  ...
  if (found) {
    if ((type == USB_DESCRIPTOR_TYPE_CONFIGURATION) ||
      ...
      *len = (p[CONF_DESC_wTotalLength]) | (p[CONF_DESC_wTotalLength + 1] << 8);
      ...
      g_usbd_core[busid].self_powered =
          (p[7] & USB_CONFIG_POWERED_MASK) ? true : false;
      g_usbd_core[busid].remote_wakeup_support =
          (p[7] & USB_CONFIG_REMOTE_WAKEUP) ? true : false;
    } else {
 ...
 */
static bool usbd_set_configuration(uint8_t busid, uint8_t config_index,
                                   uint8_t alt_setting) {
...
#ifdef CONFIG_USBDEV_ADVANCE_DESC
  p = g_usbd_core[busid].descriptors->config_descriptor_callback(
      g_usbd_core[busid].speed);
#else
  p = (uint8_t *)g_usbd_core[busid].descriptors;
#endif
  ...
  while (p[DESC_bLength] != 0U) {
    ...
    case USB_DESCRIPTOR_TYPE_CONFIGURATION:
      ...
      if (cur_config == config_index) {
        ...
        current_desc_len = 0;
        desc_len =
            (p[CONF_DESC_wTotalLength]) | (p[CONF_DESC_wTotalLength + 1] << 8);
      }
 ...
 * @return true if successfully configured false if error or unconfigured
 */
static bool usbd_set_interface(uint8_t busid, uint8_t iface,
                               uint8_t alt_setting) {
  ...
  while (p[DESC_bLength] != 0U) {
    ...
    case USB_DESCRIPTOR_TYPE_INTERFACE:
      ...
      USB_LOG_DBG("Current iface %u alt setting %u", cur_iface,
                  cur_alt_setting);
      break;
  ...
  usbd_class_event_notify_handler(busid, USBD_EVENT_SET_INTERFACE,
                                  (void *)if_desc);
 ...
 */
static bool usbd_std_device_req_handler(uint8_t busid,
 ...
 * @return true if the request was handled successfully
 */
static bool usbd_std_interface_req_handler(uint8_t busid,
                                           struct usb_setup_packet *setup,
                                           uint8_t **data, uint32_t *len) {
      ...
      for (uint8_t i = 0; i < g_usbd_core[busid].intf_offset; i++) {
        ...
        if (intf && (intf->intf_num == intf_num)) {
          *data = (uint8_t *)intf->hid_report_descriptor;
          // memcpy(*data, intf->hid_report_descriptor,
          // intf->hid_report_descriptor_len);
 ...
 * @return true if the request was handled successfully
 */
static bool usbd_std_endpoint_req_handler(uint8_t busid,
                                          struct usb_setup_packet *setup,
                                          uint8_t **data, uint32_t *len) {
 ...
 * @return true if the request was handled successfully
 */
static int usbd_standard_request_handler(uint8_t busid,
                                         struct usb_setup_packet *setup,
                                         uint8_t **data, uint32_t *len) {
  int rc = 0;
 ...
 * @return true if the request was handled successfully
 */
static int usbd_class_request_handler(uint8_t busid,
                                      struct usb_setup_packet *setup,
                                      uint8_t **data, uint32_t *len) {
  if ((setup->bmRequestType & USB_REQUEST_RECIPIENT_MASK) ==
      USB_REQUEST_RECIPIENT_INTERFACE) {
    for (uint8_t i = 0; i < g_usbd_core[busid].intf_offset; i++) {
      struct usbd_interface *intf = g_usbd_core[busid].intf[i];

      if (intf && intf->class_interface_handler &&
          (intf->intf_num == (setup->wIndex & 0xFF))) {
        return intf->class_interface_handler(busid, setup, data, len);
      }
    }
  } else if ((setup->bmRequestType & USB_REQUEST_RECIPIENT_MASK) ==
 ...
 * @return true if the request was handled successfully
 */
static int usbd_vendor_request_handler(uint8_t busid,
                                       struct usb_setup_packet *setup,
                                       uint8_t **data, uint32_t *len) {
  uint32_t desclen;
#ifdef CONFIG_USBDEV_ADVANCE_DESC
  if (g_usbd_core[busid].descriptors->msosv1_descriptor) {
    if (setup->bRequest ==
        g_usbd_core[busid].descriptors->msosv1_descriptor->vendor_code) {
      switch (setup->wIndex) {
      case 0x04:
        desclen =
            g_usbd_core[busid].descriptors->msosv1_descriptor->compat_id[0] +
            (g_usbd_core[busid].descriptors->msosv1_descriptor->compat_id[1]
             << 8) +
            (g_usbd_core[busid].descriptors->msosv1_descriptor->compat_id[2]
             << 16) +
            (g_usbd_core[busid].descriptors->msosv1_descriptor->compat_id[3]
             << 24);

        *data = (uint8_t *)g_usbd_core[busid]
                    .descriptors->msosv1_descriptor->compat_id;
        // memcpy(*data,
        // g_usbd_core[busid].descriptors->msosv1_descriptor->compat_id,
        // desclen);
        *len = desclen;
        return 0;
      case 0x05:
        desclen = g_usbd_core[busid]
                      .descriptors->msosv1_descriptor
                      ->comp_id_property[setup->wValue][0] +
                  (g_usbd_core[busid]
                       .descriptors->msosv1_descriptor
                       ->comp_id_property[setup->wValue][1]
                   << 8) +
                  (g_usbd_core[busid]
                       .descriptors->msosv1_descriptor
                       ->comp_id_property[setup->wValue][2]
                   << 16) +
                  (g_usbd_core[busid]
                       .descriptors->msosv1_descriptor
                       ->comp_id_property[setup->wValue][3]
                   << 24);

        *data = (uint8_t *)g_usbd_core[busid]
                    .descriptors->msosv1_descriptor
                    ->comp_id_property[setup->wValue];
        // memcpy(*data,
    ...
    if (setup->bRequest ==
        g_usbd_core[busid].descriptors->msosv2_descriptor->vendor_code) {
      switch (setup->wIndex) {
      case WINUSB_REQUEST_GET_DESCRIPTOR_SET:
        desclen =
            g_usbd_core[busid].descriptors->msosv2_descriptor->compat_id_len;
        *data = (uint8_t *)g_usbd_core[busid]
                    .descriptors->msosv2_descriptor->compat_id;
        // memcpy(*data,
        // g_usbd_core[busid].descriptors->msosv2_descriptor->compat_id,
        // desclen);
  ...
  if (g_usbd_core[busid].descriptors->webusb_url_descriptor) {
    if (setup->bRequest ==
        g_usbd_core[busid].descriptors->webusb_url_descriptor->vendor_code) {
      switch (setup->wIndex) {
      case WEBUSB_REQUEST_GET_URL:
        desclen =
            g_usbd_core[busid].descriptors->webusb_url_descriptor->string_len;
        *data = (uint8_t *)g_usbd_core[busid]
                    .descriptors->webusb_url_descriptor->string;
        // memcpy(*data,
        // g_usbd_core[busid].descriptors->webusb_url_descriptor->string,
        // desclen);
        *len = desclen;
        return 0;
      default:
  ...
  if (g_usbd_core[busid].msosv1_desc) {
    if (setup->bRequest == g_usbd_core[busid].msosv1_desc->vendor_code) {
      ...
      case 0x05:
        *data = (uint8_t *)g_usbd_core[busid]
                    .msosv1_desc->comp_id_property[setup->wValue];
        desclen =
            g_usbd_core[busid].msosv1_desc->comp_id_property[setup->wValue][0] +
            (g_usbd_core[busid].msosv1_desc->comp_id_property[setup->wValue][1]
             << 8) +
            (g_usbd_core[busid].msosv1_desc->comp_id_property[setup->wValue][2]
             << 16) +
            (g_usbd_core[busid].msosv1_desc->comp_id_property[setup->wValue][3]
             << 24);
        // memcpy(*data,
        // g_usbd_core[busid].msosv1_desc->comp_id_property[setup->wValue],
        // desclen);
        *len = desclen;
        return 0;
      default:
    ...
    if (setup->bRequest == g_usbd_core[busid].msosv2_desc->vendor_code) {
      ...
      case WINUSB_REQUEST_GET_DESCRIPTOR_SET:
        *data = (uint8_t *)g_usbd_core[busid].msosv2_desc->compat_id;
        // memcpy(*data, g_usbd_core[busid].msosv2_desc->compat_id,
        // g_usbd_core[busid].msosv2_desc->compat_id_len);
  ...
  for (uint8_t i = 0; i < g_usbd_core[busid].intf_offset; i++) {
    ...
    if (intf && intf->vendor_handler &&
        (intf->vendor_handler(busid, setup, data, len) == 0)) {
      return 0;
    }
  }
 ...
 */
static bool usbd_setup_request_handler(uint8_t busid,
                                       struct usb_setup_packet *setup,
                                       uint8_t **data, uint32_t *len) {
  ...
  for (uint8_t i = 0; i < g_usbd_core[busid].intf_offset; i++) {
    ...
    if (arg) {
      ...
      if (intf && intf->notify_handler &&
          (desc->bInterfaceNumber == (intf->intf_num))) {
...
static void
__usbd_event_ep0_setup_complete_handler(uint8_t busid,
                                        struct usb_setup_packet *setup) {
  uint8_t *buf;
  ...
  if (setup->wLength &&
    ...
    usbd_ep_start_read(busid, USB_CONTROL_OUT_EP0,
                       g_usbd_core[busid].ep0_data_buf, setup->wLength);
    return;
  }

  /* Ask installed handler to process request */
  if (!usbd_setup_request_handler(busid, setup, &buf,
                                  &g_usbd_core[busid].ep0_data_buf_len)) {
    g_usbd_core[busid].ep0_next_state = USBD_EP0_STATE_SETUP;
    usbd_ep_set_stall(busid, USB_CONTROL_IN_EP0);
    return;
  }
  ...
  g_usbd_core[busid].ep0_data_buf_residue =
      MIN(g_usbd_core[busid].ep0_data_buf_len, setup->wLength);
  if (g_usbd_core[busid].ep0_data_buf_residue >
  ...
  /* use *data = xxx; g_usbd_core[busid].ep0_data_buf records real data address,
   * we should copy data into ep0 buffer. Why we should copy once? because some
   * chips are not access to flash with dma if real data address is in flash
   * address(such as ch32).
   */
  if (buf != g_usbd_core[busid].ep0_data_buf) {
#ifdef CONFIG_USBDEV_EP0_INDATA_NO_COPY
    g_usbd_core[busid].ep0_data_buf = buf;
#else
    usb_memcpy(g_usbd_core[busid].ep0_data_buf, buf,
               g_usbd_core[busid].ep0_data_buf_residue);
#endif
  } else {
  ...
  usbd_ep_start_write(busid, USB_CONTROL_IN_EP0,
                      g_usbd_core[busid].ep0_data_buf,
                      g_usbd_core[busid].ep0_data_buf_residue);
  /*
   ...
   * sent.
   */
  if ((setup->wLength > g_usbd_core[busid].ep0_data_buf_len) &&
      (!(g_usbd_core[busid].ep0_data_buf_len % USB_CTRL_EP_MPS))) {
...
static void usbd_event_ep0_in_complete_handler(uint8_t busid, uint8_t ep,
                                               uint32_t nbytes) {
  struct usb_setup_packet *setup = &g_usbd_core[busid].setup;
  ...
  if (g_usbd_core[busid].ep0_data_buf_residue != 0) {
    ...
    if (g_usbd_core[busid].zlp_flag == true) {
       ...
       */
      if (setup->wLength && ((setup->bmRequestType & USB_REQUEST_DIR_MASK) ==
                             USB_REQUEST_DIR_IN)) {
...
static void usbd_event_ep0_out_complete_handler(uint8_t busid, uint8_t ep,
                                                uint32_t nbytes) {
  struct usb_setup_packet *setup = &g_usbd_core[busid].setup;
  ...
  if (nbytes > 0) {
    ...
    if (g_usbd_core[busid].ep0_data_buf_residue == 0) {
      ...
      g_usbd_core[busid].ep0_data_buf = g_usbd_core[busid].req_data;
      if (!usbd_setup_request_handler(busid, setup,
                                      &g_usbd_core[busid].ep0_data_buf,
                                      &g_usbd_core[busid].ep0_data_buf_len)) {
        g_usbd_core[busid].ep0_next_state = USBD_EP0_STATE_SETUP;
        usbd_ep_set_stall(busid, USB_CONTROL_IN_EP0);
...
void usbd_event_ep_in_complete_handler(uint8_t busid, uint8_t ep,
                                       uint32_t nbytes) {
...
void usbd_event_ep_out_complete_handler(uint8_t busid, uint8_t ep,
                                        uint32_t nbytes) {
...
void usbd_msosv1_desc_register(uint8_t busid,
                               struct usb_msosv1_descriptor *desc) {
  g_usbd_core[busid].msosv1_desc = desc;
}

/* Register MS OS Descriptors version 2 */
void usbd_msosv2_desc_register(uint8_t busid,
                               struct usb_msosv2_descriptor *desc) {
...
void usbd_webusb_desc_register(uint8_t busid,
                               struct usb_webusb_descriptor *desc) {
...
int usbd_send_remote_wakeup(uint8_t busid) {
  if (g_usbd_core[busid].remote_wakeup_support &&
      g_usbd_core[busid].remote_wakeup_enabled &&
      g_usbd_core[busid].is_suspend) {
...
static void usbdev_ep0_thread(CONFIG_USB_OSAL_THREAD_SET_ARGV) {
  ...
  while (1) {
    ret = usb_osal_mq_recv(g_usbd_core[busid].usbd_ep0_mq, (uintptr_t *)&event,
...
int usbd_initialize(uint8_t busid, uintptr_t reg_base,
                    void (*event_handler)(uint8_t busid, uint8_t event)) {
  int ret;
  struct usbd_bus *bus;
  ...
  g_usbd_core[busid].usbd_ep0_thread = usb_osal_thread_create(
      "usbd_ep0", CONFIG_USBDEV_EP0_STACKSIZE, CONFIG_USBDEV_EP0_PRIO,
      usbdev_ep0_thread, (void *)(uint32_t)busid);
  if (g_usbd_core[busid].usbd_ep0_thread == NULL) {
    USB_LOG_ERR(
        "No memory to alloc for g_usbd_core[busid].usbd_ep0_thread\r\n");
    while (1) {
    }
  }
#endif
